...
if not crypto then
	...
	keygen = 1; -- set this to 1 to generate keys, 0 normal operation
	keys = {
		["rnd"] = { -- example key 1
	...
	welcomemsg = function()
		if keygen == 1 then
			say(minetest.colorize("red", "rnd key exchange. hold w+s to generate private/public key or wait to receive one."))
		end
	end
	welcomemsg()
	
	crypto = _G.crypto; bignum = _G.bignum;
	-- mode/states description: on each step there is 5s timeout to reset to 0/0
	--A. KEY GENERATION (if keygen = 1. simply check if other party key exists yet.. public keys are saved in special array keys = {name = {key},...})
	
	-- state 1: mode 1: send G^b in 2 steps -> mode 2: wait for G^c in 2 steps -> state 2 : compute session key and send encrypted public key v = g^x
	-- state 0: mode 0: receive 1st part of G^b -> mode 1: receive 2nd part of G^b -> state 2: compute session key and v = g^x

	--B. normal chat with authentication (keygen = 0 )
	
	--	state 0 : mode 0 : idle, await encrypted(name .. y = g^r+y)-> send y^x, compute session key -> state = 1 : normal chat
	--  state 0: press w+s -> send encrypted(g^r+y). compute session key -> state 1 : normal chat
...
if keygen == 1 then -- generating & exchanging 'public' key for one of the clients
	...
	-- -> mode 3:  receive v = g^x -> keygen = 0, state = 0 (key computed, ready to proceed with chat operations)
	...
	-- mode 3 : compute session key and send encrypted public key v = g^x -> display public and private key, state = 0, keygen = 0
